...
export type OpType = 'insert' | 'deactivate' | 'upsert' | 'close' | 'delete' | 'invalidate';
...
export function computeVersions(ops: Operation[]): Version[] {
  ...
  for (const op of ordered) {
    ...
    for (const row of open) {
      ...
      // left remainder
      if (row.vs < range[0]) {
        nextOpen.push({ vs: row.vs, ve: Math.min(range[0], row.ve), ss: tx, se: INF, value: row.value, createdBy: op.id ?? -1 });
      }
      // right remainder
      if (range[1] < row.ve) {
        nextOpen.push({ vs: Math.max(range[1], row.vs), ve: row.ve, ss: tx, se: INF, value: row.value, createdBy: op.id ?? -1 });
      }
    }
    ...
    const isDeactivate = op.type === 'deactivate' || op.type === 'close' || op.type === 'delete' || op.type === 'invalidate';

    if (isInsert) {
      nextOpen.push({ vs: range[0], ve: range[1], ss: tx, se: INF, value: op.value, createdBy: op.id ?? -1 });
...
function mergeAdjacent(rows: Version[]): Version[] {
  ...
  rows.sort((a, b) => { const c1 = cmp(a.vs, b.vs); if (c1) return c1; const c2 = cmp(a.ve, b.ve); if (c2) return c2; return cmp(a.ss, b.ss); });
  const out: Version[] = [];
  for (const r of rows) {
...
export function extents(items: Version[]): { tmin: number; tmax: number; vmin: number; vmax: number } {
...
export function normalizeForDisplay(versions: Version[], ops: Operation[], txMaxOverride?: number, vMaxOverride?: number): Version[] {
  const txMax = Number.isFinite(txMaxOverride) ? (txMaxOverride as number) : Math.max(0, ...ops.map(o => o.tx));
  const vMax = Number.isFinite(vMaxOverride) ? (vMaxOverride as number) : Math.max(0, ...versions.map(v => v.ve === INF ? 0 : v.ve));
  const wallX = txMax + 1;
  const wallY = vMax + 1;
  return versions.map(v => ({ ...v, se: v.se === INF ? wallX : v.se, ve: v.ve === INF ? wallY : v.ve, _sysOpen: v.se === INF, _validOpen: v.ve === INF }));
}
