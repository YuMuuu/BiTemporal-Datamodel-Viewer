...
export function Canvas({ versions, options }: { versions: Version[]; options: CanvasOptions }) {
  ...
  const { tmin, tmax, vmin, vmax } = useMemo(() => {
    ...
    const txMax = Number.isFinite(options?.gridMaxTx)
      ? Math.max(1, options!.gridMaxTx as number)
      : ex.tmax;
    const vMax = Number.isFinite(options?.gridMaxValid)
      ? Math.max(1, options!.gridMaxValid as number)
      : ex.vmax;
    ...
    >
      ...
      {options?.showGrid && (
        <g stroke="#2b3645" strokeWidth="1" vectorEffect="non-scaling-stroke" opacity="0.35">
          {gridLines(tmin, tmax, 1).map((t) => {
            const x = xOf(t);
            return (
              <line key={`vx-${t}`} x1={x} y1={PADDING.top} x2={x} y2={height - PADDING.bottom} />
            );
          })}
          {gridLines(vmin, vmax, 1).map((v) => {
            const y = yOf(v);
            return (
              <line key={`hy-${v}`} x1={PADDING.left} y1={y} x2={width - PADDING.right} y2={y} />
            );
          })}
        </g>
      )}
      ...
      <g fill="#8b949e" fontSize="11">
        {gridLines(tmin, tmax, 1).map((t) => {
          const x = xOf(t);
          return (
            <text key={`tx-${t}`} x={x + 2} y={height - PADDING.bottom + 16}>
              {t}
            </text>
          );
        })}
        {gridLines(vmin, vmax, 1).map((v) => {
          const y = yOf(v);
          return (
            <text key={`ty-${v}`} x={PADDING.left - 24} y={y + 4}>
              {v}
            </text>
          );
        })}
      ...
      {drawVersions.map((r, idx) => {
        ...
        const activeSys =
          asOfSys == null
            ? true
            : r.ss <= asOfSys && asOfSys < (r._sysOpen ? Number.POSITIVE_INFINITY : r.se);
        const activeVal =
          asOfVal == null
            ? true
            : r.vs <= asOfVal && asOfVal < (r._validOpen ? Number.POSITIVE_INFINITY : r.ve);
